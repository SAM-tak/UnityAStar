...
namespace SAMtak.AStar.Tests
{
    public class MockEuclidPathFinder : PathFinder<float>
    {
        ...
        public MockEuclidPathFinder (short[,] grid, int bufferReserveSize = 256) : base(bufferReserveSize)
        {
        ...
        public class Node : Vector2IntEuclidNode
        {
            public MockEuclidPathFinder pathFinder;

            public override float EstimateCostTo(INode other) => base.EstimateCostTo(other) + pathFinder.Grid[position.y, position.x];

            public override IEnumerable<INode> GetNeighbors(INode goalNode)
            {
                foreach(var i in GridEnumerator.OneChebyshev(((Node)Ancestor)?.position ?? position, position, ((Node)goalNode).position)) {
